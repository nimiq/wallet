...
var ArrayUtils = class {
  static subarray(uintarr, begin = 0, end = uintarr.byteLength) {
...
var StringUtils = class _StringUtils {
  static isWellFormed(str) {
    return typeof str.isWellFormed === "function" ? str.isWellFormed() : _StringUtils._isWellFormedUnicode(str);
  }
  static isHex(str) {
   ...
   */
  static _isWellFormedUnicode(string) {
    var len = string.length;
    var k = 0;
    while (k < len) {
      var cp = _StringUtils._codePointAt(string, k);
   ...
   */
  static _codePointAt(string, position) {
    ...
    var firstIsLeading = _StringUtils._isLeadingSurrogate(first);
    var firstIsTrailing = _StringUtils._isTrailingSurrogate(first);
    ...
    if (!_StringUtils._isTrailingSurrogate(second)) {
      return {
        "[[CodePoint]]": cp,
        "[[CodeUnitCount]]": 1,
        "[[IsUnpairedSurrogate]]": true
    ...
    return {
      "[[CodePoint]]": _StringUtils._utf16SurrogatePairToCodePoint(first, second),
      "[[CodeUnitCount]]": 2,
      "[[IsUnpairedSurrogate]]": false
    };
  }
   ...
   */
  static _isLeadingSurrogate(charCode) {
   ...
   */
  static _isTrailingSurrogate(charCode) {
   ...
   */
  static _utf16SurrogatePairToCodePoint(lead, trail) {
    if (!_StringUtils._isLeadingSurrogate(lead) || !_StringUtils._isTrailingSurrogate(trail)) {
      throw new Error(
...
var SerialBuffer = class _SerialBuffer extends Uint8Array {
  ...
  readVarUint() {
    ...
    do {
      ...
      b = this.readUint8();
      res += shift < 28 ? (b & VARINT_REST) << shift : (b & VARINT_REST) * Math.pow(2, shift);
      shift += 7;
    } while (b >= VARINT_MSB);
    return res;
  }
  ...
  writeVarUint(value) {
    ...
    while (value >= VARINT_INT) {
      this.writeUint8(value & 255 | VARINT_MSB);
      value /= 128;
    }
  ...
  static varLengthStringSize(value) {
    if (!StringUtils.isWellFormed(value)) throw new Error("Malformed value");
    const bytes = BufferUtils.fromUtf8(value);
    return _SerialBuffer.varUintSize(bytes.byteLength) + bytes.byteLength;
  }
};
var VARINT_MSB = 128;
var VARINT_REST = 127;
var VARINT_MSBALL = ~VARINT_REST;
var VARINT_INT = Math.pow(2, 31);
...
var BufferUtils = class _BufferUtils {
  ...
  static _utf8TextDecoder(buf, options) {
    if (typeof TextDecoder === "undefined") throw new Error("TextDecoder not supported");
    if (_BufferUtils._UTF8_DECODER === null) throw new Error("TextDecoder does not support utf8");
    if (_BufferUtils._UTF8_DECODER === void 0) {
      try {
        _BufferUtils._UTF8_DECODER = new TextDecoder("utf-8", options);
      } catch (e) {
        _BufferUtils._UTF8_DECODER = null;
        throw new Error("TextDecoder does not support utf8");
      }
    }
    return _BufferUtils._UTF8_DECODER.decode(buf);
  }
  static toUtf8(buf) {
    return _BufferUtils._utf8TextDecoder(buf);
  }
...
// lib/Entropy.ts
import { CryptoUtils as CryptoUtils4 } from "../../web/index.js";

// lib/ExtendedPrivateKey.ts
import { CryptoUtils, PrivateKey, PublicKey } from "../../web/index.js";
...
var ExtendedPrivateKey = class _ExtendedPrivateKey extends Serializable {
   ...
   */
  static fromHex(hex) {
    return _ExtendedPrivateKey.deserialize(BufferUtils.fromHex(hex));
  }
  /**
...
import { CryptoUtils as CryptoUtils2, Hash } from "../../web/index.js";
...
var MnemonicUtils = class _MnemonicUtils {
  ...
  static mnemonicToSeed(mnemonic, password) {
    ...
    const saltBuffer = BufferUtils.fromUtf8(_MnemonicUtils._salt(password));
  ...
  static _crcChecksum(entropy) {
    ...
    return _MnemonicUtils._toBinary([hash]).slice(0, CS);
  }
  ...
  static _entropyToBits(entropy) {
    ...
    return _MnemonicUtils._toBinary(entropy);
  }
  ...
  static _bitsToMnemonic(bits, wordlist) {
    ...
    const words = chunks.map((chunk) => {
      const index = _MnemonicUtils._fromBinary(chunk);
  ...
  static _bitsToEntropy(bits, legacy = false) {
    ...
    const entropyBytes = chunks.map(_MnemonicUtils._fromBinary);
  ...
  static _fromBinary(bin) {
    return parseInt(bin, 2);
  }
  static _toBinary(buffer) {
    let bin = "";
    for (let i = 0; i < buffer.length; i++) {
      const code = buffer[i];
...
// lib/Secret.ts
import { CryptoUtils as CryptoUtils3, Hash as Hash2, PrivateKey as PrivateKey2 } from "../../web/index.js";
var Secret = class _Secret extends Serializable {
  ...
  static async exportEncrypted(secret, key) {
    const salt = CryptoUtils3.getRandomValues(_Secret.ENCRYPTION_SALT_SIZE);
    const data = new SerialBuffer(
      /*purposeId*/
      4 + _Secret.SIZE
    );
    if (secret instanceof PrivateKey2) {
      data.writeUint32(PrivateKey2.PURPOSE_ID);
  ...
  async exportEncrypted(key) {
    return _Secret.exportEncrypted(this, key);
  }
  /**
...
var Entropy = class _Entropy extends Secret {
   ...
   */
  static fromHex(hex) {
    return _Entropy.deserialize(BufferUtils.fromHex(hex));
  }
  /**
