    ...
    <Modal class="disclaimer-modal">
        <PageBody class="flex-column">
            <header class="flex-column">
                ...
                <img src="@/assets/exchanges/coinify-full.png" alt="Coinify" />
            </header>

            <main class="flex-column">
                <h1 class="nq-h1" v-if="$config.polygon.enabled && stablecoin">
                    {{ $t('Sell NIM, BTC and {ticker}', { ticker: stablecoin.toUpperCase() }) }}
                </h1>
                <h1 class="nq-h1" v-else>{{ $t('Sell NIM & BTC') }}</h1>

                <p class="subline">{{ $t('Coinify is a third-party service requiring registration and KYC.') }}</p>

                <div class="crypto-logos flex-row">
                    <span class="nq-icon nimiq-logo" title="NIM"></span>
                    <BitcoinIcon title="BTC" />
                    <UsdcIcon v-if="$config.polygon.enabled && stablecoin === CryptoCurrency.USDC"
                        :title="stablecoin.toUpperCase()"
                    />
                    <UsdtIcon v-if="$config.polygon.enabled && stablecoin === CryptoCurrency.USDT"
                        :title="stablecoin.toUpperCase()"
                    />
                </div>
        ...
        <PageFooter>
            <button class="nq-button light-blue" @click="$router.push({ name: 'coinify', params: { flow: 'sell' } })">
                {{ $t('Continue with Coinify') }}
            </button>
        </PageFooter>
    </Modal>
</template>
