<template>
    <div class="prestaking-welcome-page flex-column"
        ...
        <div class="content-wrapper" ref="contentWrapper">
            <transition name="fade" mode="out-in" @enter="onTransitionEnter" @leave="onTransitionLeave">
...
export default defineComponent({
    setup(props, { emit, root }) {
        ...
        const onTransitionLeave = (el: HTMLElement, done: () => void) => {
            if (contentWrapper.value) {
                el.style.opacity = '0';
                el.style.transform = 'scale(0.98)';
            }
            setTimeout(done, 300); // Reduced from 500ms to 300ms
        };
...
.prestaking-welcome-page {
    ...
    flex-grow: 1;
    touch-action: pan-y;
    transition: height 0.45s ease; // Add this line to animate the height
    overflow: hidden; // Add this line to hide overflow during transition
}
...
.content-wrapper {
    width: 100%;
    transition: height 0.3s ease; // Reduced from 0.5s to 0.3s
    overflow: hidden;
}
