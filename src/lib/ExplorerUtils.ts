...
export function explorerTxLink(asset: CryptoCurrency, hash: string) {
    ...
    switch (asset) {
        case CryptoCurrency.NIM:
            return `https://${config.environment === ENV_MAIN ? '' : (config.environment === 'albatross' ? 'albatross.' : 'test.')}nimiq.watch/#${hash}`;
...
export function explorerAddrLink(asset: SwapAsset, address: string) {
    ...
    switch (asset) {
        case SwapAsset.NIM:
            return `https://${config.environment === ENV_MAIN ? '' : (config.environment === 'albatross' ? 'albatross.' : 'test.')}nimiq.watch/#${address}`;
        case SwapAsset.BTC:
